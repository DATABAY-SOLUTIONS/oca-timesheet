    ...
    def _compute_group_ids(self):
        ...
        for report in self:
            ...
            if report.groupby_field_ids:
                grouped_lines = AccountAnalyticLine._read_group(
...
class HrTimesheetReportGroup(models.TransientModel):
    ...
    def _compute_entry_ids(self):
        ...
        for group in self:
            grouped_lines = AccountAnalyticLine._read_group(
                domain=safe_eval(group.scope) if group.scope else Domain.TRUE,
                fields=list(
    ...
    def _compute_any_line_id(self):
        ...
        for entry in self:
            entry.any_line_id = AccountAnalyticLine.search(
                safe_eval(entry.scope) if entry.scope else Domain.TRUE,
                limit=1,
            )
